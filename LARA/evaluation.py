...
def dcg_k(r, k):
    r = np.asfarray(r)
    # 因为这里预测是按照隐式反馈来计算的，r只有两种取值，0或者1
    # 因此等同于，原作者写的r/(np.log2(np.arange(2, r.size+2)))
    ...
    for test_i, test_user_list in zip(item, k_similar_user):
        for test_u in test_user_list:
            if ui_matrix[test_u, test_i] == 1:
                count += 1
    # p@k计算方法：计算预测的k个用户，有几个用户与item有过交互，除以k
    # 假如模型预测了10个用户，实际上只有3个用户都与item有过交互
...
def to_valuate(item, item_user):
    ...
    print("p@10:%.4f   p@20:%.4f   ndcg@10:%.4f   ndcg@20:%.4f\n" % (p10, p20, ndcg_10, ndcg_20))
